...
def dictionary(descriptors, n_clusters):
    ...
    """
    # TODO
    kmeans = MiniBatchKMeans(n_clusters=n_clusters, random_state = 0, batch_size = 100, max_iter = 10).fit(descriptors)
...
def vlad(files, mus, powernorm, gmp=False, gamma=1000):
    ...
    for f in tqdm(files):
        ...
        for k in range(mus.shape[0]):
            ...
            n_desc = desc[a[:, k] > 0]
            sum_pooling = np.zeros((desc.shape[1]))
            if len(n_desc) > 0:
                diff = n_desc - mus[k]
                sum_pooling = np.sum(diff, axis=0)

            f_enc.extend(list(sum_pooling))

        # c) power normalization
        if powernorm:
            # TODO
            f_enc = np.multiply(np.sign(f_enc),np.sqrt(np.abs(f_enc)))

        # # l2 normalization
        # # TODO
        l2_norm = np.linalg.norm(f_enc)
        e = np.array(f_enc) / l2_norm
        encodings[j] = e
        j = j + 1
...
def esvm(encs_test, encs_train, C=1000):
    ...
    # set up labels
    # TODO
    label = np.ones((len(encs_train)+1))*(-1)
    label[0] = 1
    ...
    def loop(i):
        ...
        # TODO
        matrix = np.concatenate((encs_test[i].reshape(1,-1), encs_train), axis=0)
        lin_clf = LinearSVC(C=C, class_weight='balanced').fit(matrix, label)
        coeff = lin_clf.coef_
...
def distances(encs):
    ...
    # encodings
    # TODO
    normalized_encs = encs/np.linalg.norm(encs)
    dists = np.empty((encs.shape[0],encs.shape[0]))
    for t1 in range(encs.shape[0]):
        for t2 in range(encs.shape[0]):
            d = np.dot(normalized_encs[t1], normalized_encs[t2])
            dists[t1][t2] = 1 - d
...
if __name__ == '__main__':
    ...
    if not os.path.exists('mus.pkl.gz'):
        ...
        descriptors = loadRandomDescriptors(files_train, max_descriptors)
        ...
        # TODO
        n_clusters = 100
        mus = dictionary(descriptors, n_clusters)
    ...
    if not os.path.exists(fname) or args.overwrite:
        ...
        enc_test = vlad(files_test, mus, powernorm=False, gmp=False, gamma=1000)
    ...
    if not os.path.exists(fname) or args.overwrite:
        # TODO
        enc_train = vlad(files_train, mus, powernorm=False, gmp=False, gamma=1000)
        with gzip.open(fname, 'wb') as fOut:
    ...
    # TODO
    encs_test = esvm(enc_test, enc_train, C=1000)
    # eval
    evaluate(encs_test, labels_test)
    print('> evaluate')
